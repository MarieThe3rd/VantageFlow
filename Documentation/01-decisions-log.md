...
## 20. Task dates: StartDate, DueDate, and CompletedDate replacing the stored IsComplete

A real domain gap, not just a feature request: the original motivating scenario for this whole app was "my manager asks me to do something by the end of the week" — and until now, `TaskItem` had no due date at all. Added `StartDate` and `DueDate` (both `DateOnly?`, optional — same reasoning as every other Task field: a personal task like "clean the garage" has neither).

`CompletedDate` (`DateOnly?`) replaces the previously-stored `IsComplete` bool rather than sitting alongside it. Two fields that both claim to represent "is this done" can drift out of sync (`IsComplete = false` with a `CompletedDate` still set, or vice versa); a task being complete is now defined as *having* a completion date, nothing else. `IsComplete` still exists as a derived, bindable property (`get => CompletedDate.HasValue`; `set` stamps today's date or clears it) — the checkbox in the task list keeps working exactly as before, it just now also captures *when*. Setting `CompletedDate` directly (e.g. correcting it via the edit dialog, since "I actually finished this yesterday" is a real scenario) makes `IsComplete` true as a side effect, which is the correct behavior, not an inconsistency.

Required a real schema migration (`AddTaskDates` — drops `IsComplete`, adds the three new nullable columns) and an EF Core `Ignore()` on the derived `IsComplete` property, since it has custom get/set logic and would otherwise get mapped as its own (redundant, stale) column by convention. See `Documentation/Walkthroughs/11` for the `[NotifyPropertyChangedFor]` mechanism that keeps the checkbox binding working against a derived property.

## 21. TaskState (Not Started / In Progress / Completed) and filtering — both derived, no new stored fields

A third status field would repeat the exact mistake §20 just fixed, so `TaskState` (`CONTEXT.md`'s "State") is derived from `StartDate`/`CompletedDate` alone: no `StartDate` → Not Started; `StartDate` but no `CompletedDate` → In Progress; `CompletedDate` set → Completed. Named `TaskState`, not `TaskStatus` — `System.Threading.Tasks.TaskStatus` is already in scope everywhere via the project's implicit usings, and reusing the name would create ambiguous-reference errors at every call site.

Added a matching `IsStarted` (mirrors `IsComplete` exactly — derived from `StartDate`, stamps/clears today's date) so starting a task works the same way completing one does. A new "Started" checkbox sits next to "Done" in the task list.

**Filtering** (`TaskFilter`: Active/NotStarted/InProgress/Completed/All) is a presentation concern, not a domain one — it lives in `ViewModels/`, not `Models/`, alongside `TasksViewModel`. `Tasks` (the full loaded set, unfiltered) and `FilteredTasks` (what the view actually binds to) are kept as two separate collections specifically so existing and future code that reasons about "everything that's loaded" — tests included — never has to account for whatever filter happens to be selected.

**A second, different kind of reentrancy risk, and a different fix than §16's crash**: toggling "Started" or "Done" can now move a task in or out of the *visible* (filtered) set — a genuine `FilteredTasks` collection change, not just a property change on an already-visible row. That collection is the same `ListView` the triggering `CheckBox` lives inside, which is exactly the shape of bug fixed in `Documentation/Walkthroughs/10` — but this time there's no way to eliminate the mutation the way making `TaskItem` observable did, because an item genuinely needs to appear or disappear. The correct tool here is deferring the mutation via `DispatcherQueue.TryEnqueue` to the next UI dispatch cycle, once the triggering layout pass has settled — the same "quick fix" rejected in `Documentation/Walkthroughs/10` for being a band-aid there, because here the collection change itself is required, not a workaround for a missing notification.
